...
public:
	vector <int> thePouring(vector <int> capacities, 
				...
				vector <int> toId)
	{
		vector <int> result[capacities.size()] = {};
		for(int i=0; i<fromId.size(); i++)
		{
			bottles[fromId[i]] = max(0, bottles[fromId[i]]-(bottles[toId[i]]-bottles[fromId[i]]));
			bottles[toId[i]] = min(capacities[toId[i]], bottles[fromId[i]]);
			result[i] = bottles[fromId[i]];
		}
